...
from airpilot.config import CURRENT_SCHEMA_VERSION, AppConfig, load_config, save_config
...
def test_v7_config_adds_pose_clutch_thresholds(tmp_path: Path) -> None:
    ...
    assert loaded.gestures.thumb_open_threshold == 0.82
    assert loaded.gestures.finger_bend_threshold == 1.35
    assert loaded.gestures.finger_extend_threshold == 1.70


def test_v8_config_updates_untouched_thumb_open_threshold(tmp_path: Path) -> None:
    path = tmp_path / "config.json"
    path.write_text(
        json.dumps(
            {
...
def test_v8_config_preserves_custom_thumb_open_threshold(tmp_path: Path) -> None:
    path = tmp_path / "config.json"
    path.write_text(
        json.dumps(
            {
    ...
    assert loaded.schema_version == CURRENT_SCHEMA_VERSION
    assert loaded.gestures.thumb_open_threshold == 0.88
